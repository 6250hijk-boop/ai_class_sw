...
1. **문제집 준비 (데이터 수집):** 공부할 문제들을 많이 모아요. (예: 개와 고양이 사진 수만 장)
2. **공부하기 (모델 학습):** 문제집을 열심히 풀면서 규칙을 찾아요. "귀가 뾰족하면 고양이일 확률이 높네?" 같은 규칙을 컴퓨터가 스스로 만들어요.
3. **모의고사 풀기 (예측/테스트):** 공부가 잘 됐는지 확인하기 위해, 정답을 가리고 문제를 풀어봐요.
4. **채점 및 오답노트 (평가 및 수정):** 틀린 문제가 있다면 왜 틀렸는지 분석하고, 규칙을 다시 수정해서 더 똑똑해지도록 반복해요.
5. **실전 시험 (배포):** 이제 충분히 똑똑해졌다면, 실제로 세상에 나가서 사람들의 질문에 답하거나 사진을 분류하는 일을 시작해요.
        """)
...
with tab2:
    ...
    QUIZZES = [
        {
            ...
            "answer": "지도학습",
            "explain": "지도학습은 정답(라벨)이 있는 데이터로 컴퓨터를 학습시키는 방식입니다."
        },
        {
            "q": "정답 없이 비슷한 것끼리 묶는 머신러닝 방식은?",
            "options": ["지도학습", "강화학습", "비지도학습", "전이학습"],
            "answer": "비지도학습",
            "explain": "비지도학습은 정답 없이 데이터의 패턴을 스스로 찾아 분류하는 방식입니다."
        },
        {
            "q": "머신러닝 모델 학습 과정에서 가장 먼저 해야 할 것은?",
            "options": ["모델 배포", "데이터 수집", "모델 평가", "예측"],
            "answer": "데이터 수집",
            "explain": "모델 학습을 위해서는 먼저 충분한 데이터를 수집해야 합니다."
        },
    ]
    ...
    answered = sum(1 for i in range(len(QUIZZES)) if st.session_state.get(f"ml_quiz_{i}"))

...
with tab1:
    ...
    with st.expander("과적합(Overfitting)이란 무엇인가요?"):
        st.write("학습용 문제집의 정답을 통째로 외워버린 상태예요! 문제집은 100점 맞지만, 새로운 문제(실전)에서는 빵점을 맞는 현상이죠.\n만약, 내가 자동차를 찾고 싶은데 현대, 기아차로만 학습시켰어요.\n적당히 학습시키면 모든 차를 찾을 수 있지만 \n과적합이 되면 현대, 기아차만 차만 찾을 수 있게 되는거에요. ")
...
with tab3:
    ...
    이건 예시에요! 내가 수집한 데이터양에 따라 적당한 에포크가 결정되요!! 
    무조건 이 숫자가 좋은게 아니랍니다.
    """)
